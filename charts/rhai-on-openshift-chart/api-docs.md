...
| components.kserve.gateway.spec.listeners[0].allowedRoutes.namespaces | string | `nil` | REQUIRED: set `from` to one of Same (recommended), Selector, or All. Template will fail if not set. |
...
| components.kserve.modelsAsService.gateway.spec.listeners[0].allowedRoutes.namespaces | string | `nil` | REQUIRED: set `from` to one of Same (recommended), Selector, or All. Template will fail if not set. |
